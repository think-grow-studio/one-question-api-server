...
- `payload`는 발행자·워커가 읽을 JSON 문자열이며 **도메인 행에 존재하지 않는 커맨드 파라미터만** 담는다. 담을 것이 없으면 `JobPayload.empty()`로 빈 객체 `{}`를 저장한다 — NULL 금지. 소비자(특히 Python 워커)에 null 분기가 영구히 생기는 것을 막기 위함이다. 생성 시점에 확정되는 불변 값이다. `IdempotencyKey`·`RequestHash`와 같이 `JobPayload` 값 객체로 받고 엔티티 필드는 `String`으로 푼다.
