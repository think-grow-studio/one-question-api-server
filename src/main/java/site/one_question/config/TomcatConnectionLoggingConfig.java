...
import site.one_question.logging.ResponseSizeLoggingValve;

/**
 * 내장 Tomcat 커넥터가 TCP 연결 수립/종료를 로깅하는 커스텀 프로토콜을 사용하고,
 * 응답 전송 크기(gzip 압축 후)를 로깅하는 밸브를 등록하도록 설정한다.
 *
...
public class TomcatConnectionLoggingConfig {
  ...
  public WebServerFactoryCustomizer<TomcatServletWebServerFactory> tcpConnectionLoggingCustomizer() {
    return factory -> {
      factory.setProtocol(LoggingHttp11NioProtocol.class.getName());
      factory.addEngineValves(new ResponseSizeLoggingValve());
    };
  }
}
